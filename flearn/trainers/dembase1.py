...
class DemBase(object):
    ...
    def update_generalized_model(self,node,mode="hard"):
        ...
        if childs:
            ...
            rs_ws = []
            for w_shape in self.model_shape1:
                rs_w = np.zeros(w_shape)
                rs_ws.append(rs_w)

            for child in childs:
                gmd = self.update_generalized_model(child, mode)
                # rs_w[:] += child.numb_clients * gmd[:]  # weight
                for w in range(len(self.model_shape1)):
                    rs_ws[w] += child.numb_clients * gmd[w].astype(np.float64)  # weight
            avg_ws=[]
            for w in range(len(self.model_shape1)):
                avg_w = 1.0 * rs_ws[w].astype(np.float64) /node.numb_clients
                if (mode == "hard"):
                    avg_ws.append(avg_w)
                else:
                    avg_ws.append((1-self.gamma)*node.gmodel[w].astype(np.float64)+ self.gamma * avg_w)

            node.gmodel = tuple(avg_ws)
            return node.gmodel
        elif(node._type.upper()=="CLIENT"): #Client
            md = node.gmodel
            # for w in range(len(self.model_shape1)):
            #     md[w] = md[w].astype(np.float64)
    ...
    def get_hierrachical_params(self,client):
        ...
        return [h/nf for h in hmd] #normalized version
