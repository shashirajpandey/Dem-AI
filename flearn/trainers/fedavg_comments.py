...
class Server(BaseFedarated):
    def __init__(self, params, learner, dataset):
        ...
        if(params["lamb"] > 0): #lamb for proximal term
            self.inner_opt = PROXSGD(params['learning_rate'], params["lamb"])
        else: #using tf default optimizer
    ...
    def train(self):
        ...
        for i in range(self.num_rounds):
            ...
            if i % self.eval_every == 0: #evaluation stage
                ...
                num_samples = []
                local_grads = []
                #Each client c train the data
                for c in self.clients:
                    num, client_grad = c.get_grads(model_len) #get number of samples, and client gradient
                ...
                for idx in range(len(self.clients)):
                    difference += np.sum(np.square(global_grads - local_grads[idx])) #??? gradient difference for what?
            ...
            csolns = [] # collection of receiving client solutions (learning weights)
            for c in tqdm(selected_clients, desc='Client: ', leave=False, ncols=120):
                # communicate the latest model
                c.set_params(self.latest_model)

                # solve local learning problem and return (self.num_samples, soln), (self.num_samples,grad), (bytes_w, comp, bytes_r)
