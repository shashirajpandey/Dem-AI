...
DATASETS = ['nist', 'mnist', 'fmnist','logistic_synthetic']  # NIST is EMNIST in the paper
...
def read_options(num_users=5, loc_ep=10, Numb_Glob_Iters=100, lamb=0, learning_rate=0.01, alg='fedprox', weight=True, batch_size=0, dataset="mnist"):
    ...
    import flearn.models.logistic_synthetic.mclr as mclr
    mod = mclr
...
if __name__ == '__main__':
    ...
    DATA_SET = "logistic_synthetic"
    number_users = 30
    number_global_round = 600
    for i in range(len(algorithms_list)):
         main(num_users=number_users, loc_ep=local_ep[i], Numb_Glob_Iters=number_global_round, lamb=lamb_value[i],
            learning_rate=learning_rate[i], alg=algorithms_list[i], batch_size=batch_size[i], dataset=DATA_SET)

    plot_summary_one_figure2(num_users=number_users, loc_ep1=local_ep, Numb_Glob_Iters=number_global_round, lamb=lamb_value,
